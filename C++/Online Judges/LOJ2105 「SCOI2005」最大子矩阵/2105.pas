...
begin
  if (i<0) or (j<0) or (k<0) then
    ...
    for l:=0 to min(i,j)-longint(i=j) do f[i,j,k]:=max(f[i,j,k],dp(l,l,k-1)+sum[i,1]-sum[l,1]+sum[j,1]-sum[l,2]);
    dp:=f[i,j,k];
  end;
end;
begin
  ...
  for i:=1 to n do for j:=1 to m do inc(sum[i,j],sum[i-1,j]);
  writeln(dp(n,m,q));
  for i:=0 to n do
    for j:=0 to n do
      for k:=0 to q do
