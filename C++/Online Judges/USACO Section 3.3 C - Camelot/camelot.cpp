...
const int EPS=2, INF=0x3f3f3f3f;
int dis[32][32][32][32];
...
int main(){
    ...
    for(i=1; i<=n; i++){
        for(j=1; j<=m; j++){
            sum=0;
            for(k=1; k<=knightn and sum<INF; k++) sum+=dis[knightx[k]][knighty[k]][i][j];
            if(sum>=INF) continue;
            ans=min(ans, sum+max(abs(kingx-i), abs(kingy-j)));
            for(k=1; k<=knightn; k++){
                for(l=max(1, kingx-EPS); l<=kingx+EPS and l<=n; l++){
                    for(o=max(1, kingy-EPS); o<=kingy+EPS and o<=m; o++){
                        ans=min(ans, sum
                                    -dis[knightx[k]][knighty[k]][i][j]
                                    +dis[knightx[k]][knighty[k]][l][o]
                                    +max(abs(kingx-l), abs(kingy-o))
                                    +dis[l][o][i][j]);
                    }
                }
            }
